        ...
        if (gemsCollected >= numberOfGems) {
            ...
            scoreText.setText('Score: ' + score);
                if (score>=level ** 2 *100) {
                    ...
                    gameStatusText = this.add.text(50, 200, 'Your a complete failure. Try again.', { fontSize: '25px', fill: '#000' });
                    gameOver = true;
                }
...
function collectStar(player, star) {
    ...
    if (numberOfStars < level) {
        ...
        console.log ("numberOfStars is " + numberOfStars);
        }
...
function addBlock(startX, startY, width, height, color) {
    platforms.add(scene.add.rectangle(startX + width / 2, startY + height / 2, width, height, color));
}

function createTeleporter() {
    addBlock(50, 495, 30, 5, 0x3333ff); //blue teleporter
    addBlock(675, 495, 30, 5, 0xff0000); //red teleporter
    }
    ...
    if (!gameOver) {
        ...
        if (level >= 3) {
            if (enemyDestroyed == false) {
                ...
                if (playerFrozen && bombTrailAllowed) {
                    bombTrailAllowed = false;
                    setTimeout (bombTrail, 2000);
                }
